...
class GenerateRequest(BaseModel):
    ...
    formatId: str = Field(
        ..., min_length=1, description="Format ID (must be publishable)"
    )
    formatVersion: Optional[str] = Field(
        default=None, description="Optional format version hash"
    )
    mode: str = Field(
        default="simulation", description="Generation mode: simulation or production"
    )
    values: Dict[str, Any] = Field(
        default_factory=dict, description="User values for placeholders"
    )
    aiResult: Optional[AIResult] = Field(
        default=None, description="AI-generated content sections"
    )
...
async def generate_document(request: GenerateRequest):
    ...
    ai_result_dict = None
    if request.aiResult:
        ai_result_dict = {"sections": serialize_ai_sections(request.aiResult.sections)}
