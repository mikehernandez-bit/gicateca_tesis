...
class FormatField(BaseModel):
    ...
    options: Optional[List[str]] = Field(
        default=None, description="Opciones si type=select"
    )
    ...
    def validate_select_options(self) -> "FormatField":
        ...
        if self.type == FieldType.SELECT:
            if not self.options or len(self.options) == 0:
                raise ValueError(
                    "Field type 'select' requires non-empty 'options' list"
                )
        else:
            if self.options is not None:
                raise ValueError(
                    f"Field type '{self.type.value}' must not have 'options'"
                )
        return self
...
class AssetRef(BaseModel):
    ...
    kind: str = Field(
        ..., min_length=1, description="Tipo: logo, image, signature, etc."
    )
...
class RuleSet(BaseModel):
    ...
    margins: Optional[Dict[str, float]] = Field(
        default=None, description="Márgenes en cm"
    )
    ...
    extra: Optional[Dict[str, Any]] = Field(
        default=None, description="Reglas adicionales"
    )
...
class FormatSummary(BaseModel):
    ...
    university: str = Field(
        ..., min_length=1, description="Código de universidad (unac, uni, etc.)"
    )
    category: str = Field(
        ..., min_length=1, description="Categoría (general, especifica, etc.)"
    )
...
class FormatDetail(BaseModel):
    ...
    templateRef: Optional[TemplateRef] = Field(
        default=None, description="Referencia a plantilla"
    )
    fields: List[FormatField] = Field(
        default_factory=list, description="Campos del wizard"
    )
...
class CatalogVersionResponse(BaseModel):
    ...
    generatedAt: str = Field(
        ..., min_length=1, description="Timestamp ISO de generación"
    )
...
class FormatValidationError(BaseModel):
    ...
    error_type: str = Field(
        ..., description="Tipo de error (missing_field, invalid_type, etc.)"
    )
...
class CatalogValidationResponse(BaseModel):
    ...
    errors: List[FormatValidationError] = Field(
