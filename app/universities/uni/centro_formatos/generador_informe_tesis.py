"""
Archivo: app/universities/uni/centro_formatos/generador_informe_tesis.py
Proposito:
- Genera documentos DOCX para el formato "informe" de UNI.
...
def configurar_seccion_uni(section):
...
def crear_caratula_dinamica(doc, data):
    ...
    r_l = os.path.join(STATIC_ASSETS_DIR, "LogoUNI.png")
...
def generar_documento_core(ruta_json, ruta_salida):
    ...
    # 1. CARÁTULA
    configurar_seccion_uni(doc.sections[0]); crear_caratula_dinamica(doc, data)
    # 2. HOJA DE RESPETO
    doc.add_section(WD_SECTION.NEW_PAGE); configurar_seccion_uni(doc.sections[-1]); doc.add_paragraph("")
    # 3. PRELIMINARES
    doc.add_section(WD_SECTION.NEW_PAGE); configurar_seccion_uni(doc.sections[-1]); agregar_preliminares_dinamico(doc, data)
    # 4. CUERPO
    doc.add_section(WD_SECTION.NEW_PAGE); configurar_seccion_uni(doc.sections[-1]); agregar_cuerpo_dinamico(doc, data)
    # 5. FINALES
