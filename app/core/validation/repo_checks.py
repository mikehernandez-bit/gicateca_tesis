...
def check_id_collisions(format_files: List[Tuple[Path, Dict]]) -> List[Issue]:
    """
...
def check_providers_registered(
    uni_folders: List[str], registered_codes: Set[str]
) -> List[Issue]:
    """
...
def check_assets_exist(
    app_root: Path, required_assets: List[str] = None
) -> List[Issue]:
    """
    ...
    if required_assets is None:
        required_assets = ["static/assets/LogoGeneric.png"]
