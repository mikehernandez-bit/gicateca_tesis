...
def render_caratula_unac_maestria(doc: Document, block: Block) -> None:
    ...
    def _find_val(key: str, default: str) -> str:
        candidates = [c.get(key), data.get(key), (data.get("values") or {}).get(key)]
        return next(
            (str(v).strip() for v in candidates if v and str(v).strip()), default
        )

    # Universidad 18pt
    add_p_centered(
        doc,
        _find_val("universidad", "UNIVERSIDAD NACIONAL DEL CALLAO"),
        bold=True,
        size=18,
        space_after=6,
    )
    ...
    add_p_centered(
        doc,
        _find_val("facultad", "ESCUELA DE POSGRADO"),
        bold=True,
        size=16,
        space_after=6,
    )
    ...
    add_p_centered(
        doc,
        _find_val("escuela", "UNIDAD DE POSGRADO"),
        bold=True,
        size=16,
        space_after=24,
    )
    ...
    titulo = _find_val(
        "titulo", _find_val("titulo_investigacion", "[Título de la tesis]")
    )
    add_p_centered(
        doc, titulo.upper(), bold=True, size=14, space_before=24, space_after=30
    )
    ...
    asesor = c.get("asesor") or _find_val(
        "asesor_valor", _find_val("asesor_nombres", "[Apellidos y nombres]")
    )
